...
<script>
    ...
    $(".create_vendor_btn").click(function () {
        ...
        if (userFirstName == '') {
            ...
            $(".error_top").show();
            $(".error_top").html("");
            $(".error_top").append("<p>Please enter IFSC Code</p>");
            window.scrollTo(0, 0);
        } else if ($("#accountType").val() == '') {
            $(".error_top").show();
            $(".error_top").html("");
            $(".error_top").append("<p>Please select Account Type</p>");
            window.scrollTo(0, 0);
        } else {
        ...
        reader.onload = (function (theFile) {
            return function (e) {
                ...
                uploadTask.on('state_changed', function (snapshot) {
                    ...
                    jQuery("#uploding_image_owner").text("Upload failed: " + error.message);
                }, function () {
        ...
        reader.onload = (function (theFile) {
            return function (e) {
                ...
                uploadTask.on('state_changed', function (snapshot) {
                    ...
                    console.error('Upload failed:', error);
                    if (type == "photo") {
                        jQuery("#uploding_image_vendor").text("Upload failed: " + error.message);
                    } else {
                        jQuery("#uploding_image_photos").text("Upload failed: " + error.message);
                    }
                }, function () {
                    uploadTask.snapshot.ref.getDownloadURL().then(function (downloadURL) {
        ...
        reader.onload = (function (theFile) {
            return function (e) {
                ...
                uploadTask.on('state_changed', function (snapshot) {
                    ...
                    console.error('Upload failed:', error);
                    jQuery("#uploaded_image_menu").text("Upload failed: " + error.message);
                }, function () {
                    uploadTask.snapshot.ref.getDownloadURL().then(function (downloadURL) {

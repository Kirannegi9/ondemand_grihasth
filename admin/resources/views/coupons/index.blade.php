...
<script type="text/javascript">
            ...
            ajax: async function (data, callback, settings) {
                ...
                await ref.get().then(async function (querySnapshot) {
                    ...
                    await Promise.all(querySnapshot.docs.map(async (doc) => {
                        ...
                        if (childData.hasOwnProperty("vendorIDs") && Array.isArray(childData.vendorIDs)) {
                            
                            // Store both IDs and names for creating links
                            childData.storesList = childData.vendorIDs.map(vendorId => ({
                                id: vendorId,
                                name: storeNames[vendorId] || ''
                            })).filter(store => store.name !== '');
                            
                            childData.store = childData.storesList.map(s => s.name).join(', ');
                            childData.storeCount = childData.storesList.length;

                        } else if (childData.hasOwnProperty("vendorID")) {

                            childData.store = storeNames[childData.vendorID] || '';
                            childData.storeCount = 1;

                        } else {

                            childData.store = '';
                            childData.storeCount = 0;
    ...
    async function buildHTML(val) {
        ...
        // Handle multiple stores display
        if (val.hasOwnProperty("vendorIDs") && Array.isArray(val.vendorIDs) && val.vendorIDs.length > 0) {
            
            if (val.storeCount > 3) {
                // If more than 3 stores, show count as clickable button
                var storeLinksHtml = val.storesList.map(function(store) {
                    var storeRoute = '{{route("vendors.view",":id")}}';
                    storeRoute = storeRoute.replace(':id', store.id);
                    return '<a href="' + storeRoute + '" class="redirecttopage">' + store.name + '</a>';
                }).join(', ');
                html.push('<span class="view-stores-btn" data-stores="' + storeLinksHtml.replace(/"/g, '&quot;') + '" style="cursor: pointer; color: #3c8dbc; text-decoration: underline; font-weight: 500;">' + val.storeCount + ' stores (click to view)</span>');
            } else {
                // Show all store names as clickable links
                var storeLinks = val.storesList.map(function(store) {
                    var storeRoute = '{{route("vendors.view",":id")}}';
                    storeRoute = storeRoute.replace(':id', store.id);
                    return '<a href="' + storeRoute + '" class="redirecttopage">' + store.name + '</a>';
                }).join(', ');
                html.push(storeLinks);
            }

        } else if (val.hasOwnProperty("vendorID")) {
    ...
    // Handle click on "X stores" to show all stores in a modal
    $(document).on("click", ".view-stores-btn", function(e) {
        e.preventDefault();
        e.stopPropagation();
        
        var storesHtml = $(this).attr('data-stores');
        
        Swal.fire({
            title: 'Stores with this Coupon',
            html: '<div style="text-align: left; padding: 10px;">' + storesHtml + '</div>',
            icon: 'info',

                        ...
                        <fieldset>
                            ...
                            <div class="form-group row width-50">
                             ...
                             <div class="form-group row width-50">
                                ...
                                <label class="col-3 control-label">{{trans('SKU')}}</label>
                            ...
                            <div class="form-group row width-50">
                             ...
                             <div class="form-group row width-50">
                                ...
                                <label class="col-3 control-label">{{trans('Subcategory')}}</label>

                                <div class="col-7">

                                    <select id='item_subcategory' class="form-control" required>
                            ...
                            <div class="form-group row width-50">
                                ...
                                <label class="col-3 control-label">{{trans('Unit')}}</label>
                            ...
                            <div class="form-group row width-50">
                                ...
                                <label class="col-3 control-label">{{trans('Warranty/Guarantee')}}</label>

                                <div class="col-7">

                                    <input type="text" class="form-control warranty" required>
                            ...
                            <div class="form-group row width-50 brandDiv">
...
@section('scripts')
    ...
    var subcategories = database.collection('vendor_subcategories').where('section_id', '==', getCookie('section_id'));
} else {
    var vendors = database.collection('vendor_categories');
    var brand = database.collection('brands');
    var subcategories = database.collection('vendor_subcategories');
}


// ---- Load Categories ----
vendors.where('publish', '==', true).get().then(function (snapshots) {
    snapshots.docs.forEach((listval) => {
        var data = listval.data();
        ...
        if (localStorage.getItem('vendor_section_id') == data.section_id) {
            $('#item_category').append(
                $("<option></option>")
                    .attr("value", listval.id)   // use doc.id as category id
...
$('#item_category').on('change', function () {
    ...
    $('#item_subcategory').empty().append(
        $("<option></option>").attr("value", "").text("Select Subcategory")
    );
    ...
    database.collection('vendor_subcategories')
        ...
        .then(function (snapshots) {
            snapshots.docs.forEach(function (listval) {
                var data = listval.data();
                subcategories_list.push(data);

                $('#item_subcategory').append(
                    $("<option></option>")
                        .attr("value", listval.id)   // subcategory id
                        .attr("category_id", data.category_id)
                        .text(data.title)           // subcategory title
                );
            });
        });
});
...
var subcategory_category_id = $('#item_subcategory').find('option:selected').attr('category_id');
                    ...
                    if (name == '') {
                        ...
                        $(".error_top").append("<p>{{trans('lang.select_item_subcategory_error')}}</p>");
                        ...
                        if ($('#variants').val().length > 0) {
                            ...
                            await storeVariantImageData().then(async (vIMG) => {
                                ...
                                $.each(variantsSet, function (key, variant) {
                                    ...
                                    var variant_dis_price = $('#dis_price_' + variant).val() || '';
...
// function change_subcategories(selected_category) {

//     $('#item_subcategory').html('');
//     $('#item_subcategory').append($('<option value="">{{trans("lang.select_subcategory")}}</option>'));

//     subcategories_list.forEach((data) => {
//         var section_id = "";
//         var category_id = "";
...
//             $('#item_subcategory').append($("<option></option>")
//                 .attr("value", data.id)
//                 .attr("section_id", section_id)
//                 .attr("category_id", category_id) // keep track of parent
//                 .text(data.title)
//             );
//         }
//     });
...
//     // Populate subcategories for the chosen category
//     change_subcategories(category_id);

//     // Existing brand logic (unchanged)
//     if (section_id) {
...
// });
            ...
            if (item_attribute.length > 0) {
                ...
                if (attributeSet.length > 0) {
                    ...
                    $.each(variants, function (index, variant) {
                        ...
                        var check_variant_dis_price = $('#dis_price_' + variant).val() ? $('#dis_price_' + variant).val() : '';

                        html += '<input type="number" id="dis_price_' + variant + '" value="' + check_variant_dis_price + '" min="0" class="form-control" placeholder="Optional">';
    ...
    Array.from(files).forEach(function (file) {
        ...
        reader.onload = function (e) {
            ...
            // Get extension and filename safely
            var ext = file.name.split('.').pop();               // jpg, png etc.
            var nameWithoutExt = file.name.split('.')[0];       // filename only
            var timestamp = Date.now();
            var filename = nameWithoutExt + "_" + timestamp + "." + ext;

            product_image_filename.push(filename);
            productImagesCount++;

            var photos_html = `
                <span class="image-item" id="photo_${productImagesCount}">
                    <span class="remove-btn" data-id="${productImagesCount}" data-img="${base64str}">
                        <i class="fa fa-remove"></i>
                    </span>
                    <img class="rounded" width="80px" height="auto" src="${base64str}">
                </span>
            `;
    ...
    // reset input so same file can be reselected again
    $(this).val('');
});
        ...
        async function storeProductImageData() {
            ...
            if (photos.length > 0) {
                ...
                await Promise.all(photos.map(async (productPhoto, index) => {
                    ...
                    productPhoto = convertBase64UrlToBase64(productPhoto);
            ...
            try {
                ...
                if (digital_product_file != '') {
                    ...
                    if (digital_product_ext == 'zip' || digital_product_ext == "pdf") {
                        ...
                        // Convert base64url to standard base64
                        digital_product_file = convertBase64UrlToBase64(digital_product_file);
        ...
        async function storeVariantImageData() {
            ...
            if (variant_photos.length > 0) {
                ...
                await Promise.all(variant_photos.map(async (variantPhoto, index) => {
                    ...
                    // Convert base64url to standard base64
                    variantPhoto = convertBase64UrlToBase64(variantPhoto);

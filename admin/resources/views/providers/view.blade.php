                                    ...
                                    <div class="d-flex mb-1">
                                        ...
                                        <div class="sis-content pl-4">
                                            <ul class="p-0 info-list mb-0">
                                                ...
                                                <li class="d-flex align-items-center mb-2 mr-1">
                                                    <label class="mb-0 font-wi font-semibold text-dark-2">IFSC Code</label>
                                                    <span class="ifsc_code"> </span>
                                                </li>
                                                <li class="d-flex align-items-center mb-2 mr-1">
                                                    <label class="mb-0 font-wi font-semibold text-dark-2">Account Type</label>
        ...
        $(document).ready(async function() {
            ...
            await ref.get().then(async function(snapshots) {
                if (snapshots.docs.length > 0) {
                    ...
                    // Display IFSC Code and Account Type
                    if (user.hasOwnProperty('userBankDetails') && user.userBankDetails) {
                        var bankDetails = user.userBankDetails;
                        
                        if (bankDetails.hasOwnProperty('ifscCode') && bankDetails.ifscCode) {
                            $('.ifsc_code').text(bankDetails.ifscCode);
                        } else {
                            $('.ifsc_code').text('-');
                        }
                        
                        if (bankDetails.hasOwnProperty('accountType') && bankDetails.accountType) {
                            var accountTypeText = bankDetails.accountType === 'savings' ? 'Savings Account' : 
                                                bankDetails.accountType === 'current' ? 'Current Account' : 
                                                bankDetails.accountType;
                            $('.account_type').text(accountTypeText);
                        } else {
                            $('.account_type').text('-');
                        }
                    } else {

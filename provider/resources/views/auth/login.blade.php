...
<?php if (isset($_COOKIE['provider_panel_color'])) { ?>
    <style type="text/css">
        ...
        .form-material .form-control, .form-material .form-control.focus, .form-material .form-control:focus {
            background-image: linear-gradient(<?php echo $_COOKIE['provider_panel_color']; ?>, <?php echo $_COOKIE['provider_panel_color']; ?>), linear-gradient(rgba(120, 130, 140, 0.13), rgba(120, 130, 140, 0.13));
        }
...
<section id="wrapper">
    ...
    <div class="login-register" <?php if (isset($_COOKIE['provider_panel_color'])){ ?>
         style="background-color:<?php echo $_COOKIE['provider_panel_color']; ?>; <?php } ?>">
        ...
        <div class="login-logo text-center py-3" style="margin-top:5%;">
            ...
            <a href="#" style="display: inline-block;background: #fff;padding: 10px;border-radius: 5px;"><img
                        id="provider_logo" src="{{ asset('images/logo_web.png') }}" alt="Provider Logo"> </a>
...
<script type="text/javascript">
    ...
    function loginClick() {
        ...
        if(email == ''){
            ...
            $("#password_required").append("<p>{{trans('lang.email_blank')}}</p>");
        }
        else if(atposition<1 || dotposition<atposition+2 || dotposition+2>=email.length)
        {
            $("#password_required").show();
            $("#password_required").html("");
            $("#password_required").append("<p>{{trans('lang.email_valid')}}</p>");
        }
        else if(password == '')
        {
            $("#password_required").show();
            $("#password_required").html("");
            $("#password_required").append("<p>{{trans('lang.password_blank')}}</p>");
        }
        else{
            ...
            database.collection("users").where("email", "==", userEmail).get().then(async function (snapshots) {
                ...
                if (userData.active == true) {
                    if (userData.role == "provider") {
                        setCookie('login_type', 'email', 365);
        ...
        .catch(function (error) {
            ...
            // Check if error has a code property for specific error handling
            if (error.code) {
                switch(error.code) {
            ...
            $("#password_required").html('<div class="alert alert-danger" role="alert" style="margin-top: 10px; padding: 10px; border-radius: 5px;">' + errorMessage + '</div>');
        });
    ...
    function sendOTP() {
        ...
        if (jQuery("#phone").val() && jQuery("#country_selector").val()) {
            ...
            database.collection("users").where("phoneNumber", "==", phoneNumber).where("role", "==", 'provider').where("active", "==", true).get().then(async function (snapshots) {
    ...
    function applicationVerifier() {
        window.confirmationResult.confirm(document.getElementById("verificationcode").value)
                ...
                database.collection("users").where('phoneNumber', "==", result.user.phoneNumber).get().then(async function (snapshots_login) {
                    ...
                    if (userData) {
                        if (userData.role == "provider" && userData.active == true) {
                            setCookie('login_type', 'phone', 365);
                            var uid = userData.id;
                            var user = userData.id;
                            var firstName = userData.firstName;
            ...
            jQuery("#password_required_new").html("Invalid OTP");
        });
    }
    ...
    $(document).ready(function () {
        ref.get().then(async function (snapshots) {
            ...
            // Debug: Log all available settings
            console.log('Available settings:', globalSettings);
            
            // Load provider logo from Firebase - try different possible field names
            if (globalSettings.providerLogo) {
                console.log('Using providerLogo:', globalSettings.providerLogo);
                $('#provider_logo').attr('src', globalSettings.providerLogo);
            } else if (globalSettings.provider_logo) {
                console.log('Using provider_logo:', globalSettings.provider_logo);
                $('#provider_logo').attr('src', globalSettings.provider_logo);
            } else if (globalSettings.appLogo) {
                console.log('Using appLogo:', globalSettings.appLogo);
                $('#provider_logo').attr('src', globalSettings.appLogo);
            } else {
                console.log('No provider logo found in settings');
            }
        })

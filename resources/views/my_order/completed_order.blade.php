        ...
        completedorsersref.get().then(async function(completedorderSnapshots) {
            ...
            if (orderDetails.author.id != user_uuid) {
                ...
                for (let i = 0; i < orderDetails.products.length; i++) {
                    ...
                    var productPriceToUse = parseFloat(orderDetails.products[i]['price']);
                    var productOriginalPrice = productPriceToUse;
                    
                    if (orderDetails.products[i]['variant_info'] && 
                        orderDetails.products[i]['variant_info']['variant_dis_price'] && 
                        orderDetails.products[i]['variant_info']['variant_dis_price'] !== '' && 
                        orderDetails.products[i]['variant_info']['variant_dis_price'] !== '0') {
                        productPriceToUse = parseFloat(orderDetails.products[i]['variant_info']['variant_dis_price']);
                        productOriginalPrice = parseFloat(orderDetails.products[i]['variant_info']['variant_price']);
                    } else if (orderDetails.products[i]['discountPrice'] && 
                               orderDetails.products[i]['discountPrice'] !== '' && 
                               orderDetails.products[i]['discountPrice'] !== '0') {
                        productPriceToUse = parseFloat(orderDetails.products[i]['discountPrice']);
                        productOriginalPrice = parseFloat(orderDetails.products[i]['price']);
                    }

                    order_subtotal = order_subtotal + parseFloat(productPriceToUse) * parseFloat(orderDetails.products[i]['quantity']);

                    var productPriceTotal = parseFloat(productPriceToUse) * parseFloat(orderDetails.products[i]['quantity']);
                    ...
                    products_price = "";
                    var products_original_price = "";

                    productPriceTotal_val = "";

                    productExtras_val = "";

                    var hasDiscount = (productOriginalPrice > productPriceToUse);

                    if (currencyAtRight) {

                        products_price = parseFloat(productPriceToUse).toFixed(decimal_degits) + "" + currentCurrency;
                        if (hasDiscount) {
                            products_original_price = parseFloat(productOriginalPrice).toFixed(decimal_degits) + "" + currentCurrency;
                        }
                        ...
                        products_price = currentCurrency + "" + parseFloat(productPriceToUse).toFixed(decimal_degits);
                        if (hasDiscount) {
                            products_original_price = currentCurrency + "" + parseFloat(productOriginalPrice).toFixed(decimal_degits);
                        }
                    ...
                    // Format price display with discount if available
                    if (hasDiscount) {
                        products_price = '<span style="text-decoration: line-through; color: #999; margin-right: 5px;">' + products_original_price + '</span><span style="color: #28a745; font-weight: bold;">' + products_price + '</span>';
                    }

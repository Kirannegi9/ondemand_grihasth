            ...
            alldata.forEach((listval) => {
                ...
                if (val.status == "Order Completed") {
                    ...
                    for (let i = 0; i < val.products.length; i++) {
                        ...
                        // Calculate product price (variant discount or regular)
                        var productPriceForInput = parseFloat(val.products[i]['price']);
                        var productOriginalPriceForInput = productPriceForInput;
                        
                        if (val.products[i]['variant_info'] && 
                            val.products[i]['variant_info']['variant_dis_price'] && 
                            val.products[i]['variant_info']['variant_dis_price'] !== '' && 
                            val.products[i]['variant_info']['variant_dis_price'] !== '0') {
                            productPriceForInput = parseFloat(val.products[i]['variant_info']['variant_dis_price']);
                            productOriginalPriceForInput = parseFloat(val.products[i]['variant_info']['variant_price']);
                        } else if (val.products[i]['discountPrice'] && 
                                   val.products[i]['discountPrice'] !== '' && 
                                   val.products[i]['discountPrice'] !== '0') {
                            productPriceForInput = parseFloat(val.products[i]['discountPrice']);
                            productOriginalPriceForInput = parseFloat(val.products[i]['price']);
                        }

                        html = html + '<input type="hidden" class="price" value="' + parseFloat(productPriceForInput) + '">';
                        ...
                        html = html + '<input type="hidden" class="item_price" value="' + parseFloat(productOriginalPriceForInput) + '">';
                        ...
                        if (val.products[i]['variant_info']) {
                            ...
                            html = html + '<input type="hidden" class="variant_info" value="' + btoa(unescape(encodeURIComponent(JSON.stringify(val.products[i]['variant_info'])))) + '">';
            ...
            alldata.forEach((listval) => {
                ...
                if (val.status == "Order Placed" || val.status == "Order Accepted" || val.status == "Driver Pending" || val.status == "Order Shipped" || val.status == "In Transit") {
                    ...
                    for (let i = 0; i < val.products.length; i++) {
                        ...
                        var productPriceToUse = parseFloat(val.products[i]['price']);
                        var productOriginalPrice = productPriceToUse;
                        
                        if (val.products[i]['variant_info'] && 
                            val.products[i]['variant_info']['variant_dis_price'] && 
                            val.products[i]['variant_info']['variant_dis_price'] !== '' && 
                            val.products[i]['variant_info']['variant_dis_price'] !== '0') {
                            productPriceToUse = parseFloat(val.products[i]['variant_info']['variant_dis_price']);
                            productOriginalPrice = parseFloat(val.products[i]['variant_info']['variant_price']);
                        } else if (val.products[i]['discountPrice'] && 
                                   val.products[i]['discountPrice'] !== '' && 
                                   val.products[i]['discountPrice'] !== '0') {
                            productPriceToUse = parseFloat(val.products[i]['discountPrice']);
                            productOriginalPrice = parseFloat(val.products[i]['price']);
                        }

                        order_subtotal = order_subtotal + parseFloat(productPriceToUse) * parseFloat(val.products[i]['quantity']);

                        var productPriceTotal = parseFloat(productPriceToUse) * parseFloat(val.products[i]['quantity']);
                        ...
                        price = price + parseFloat(productPriceToUse) * val.products[i]['quantity'];

...
<script type="text/javascript">
    ...
    async function getSubcategories(categoryId) {
        ...
        subcatsRef.get().then(async function(snapshots) {
            ...
            if (filteredSubcategories.length > 0) {
                ...
                subcategoryHtml += '<h3>{{ trans("lang.categories") }}</h3>';
    ...
    function getProducts(categoryId, subcategoryId) {
        ...
        productsRef.get().then(async function(snapshots) {
            ...
            if (filteredProducts.length > 0) {
                ...
                productHtml = '<p class="text-center font-weight-bold mt-2">{{ trans("lang.no_results") }}</p>';
            }
    ...
    function buildProductHTML(product) {
        ...
        var product_detail_link = "{{ route('productdetail', ':id') }}".replace(':id', product.id);
    ...
    async function getVendorDetails() {
        ...
        vendorDetailsRef.get().then(async function(vendorSnapshots) {
            ...
            if (!isVendorInUserLocation(vendorDetails)) {
                ...
                $("#product-list").html('<div class="col-12 text-center py-5"><h5>{{ trans("lang.vendor_not_available_in_your_location") }}</h5><p>{{ trans("lang.please_change_your_location") }}</p></div>');
                $("#category-list").html('');
                jQuery("#overlay").hide();
                return;
            }

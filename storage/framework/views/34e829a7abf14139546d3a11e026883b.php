...
<script type="text/javascript">
    ...
    $(document).ready(async function () {
        ...
        $(document).on("click", '.add-to-cart', async function (event) {
            ...
            if ($('#variation_info_' + id).length > 0) {
                ...
                var variant_dis_price = element.attr('data-vdisprice');
                ...
                variant_info['variant_dis_price'] = variant_dis_price && variant_dis_price !== '' ? parseFloat(variant_dis_price) : '';
                variant_info['variant_qty'] = variant_qty;
                variant_info['variant_image'] = variant_img;
                // Use discount price if available, otherwise use regular price
                if (variant_dis_price && variant_dis_price !== '' && variant_dis_price !== '0') {
                    item_price = parseFloat(variant_dis_price);
                    price = parseFloat(variant_dis_price);
                    dis_price = parseFloat(variant_dis_price);
                } else {
        ...
        vendorDetailsRef.get().then(async function (vendorSnapshots) {
            ...
            if (sectionName == 'Multivendor Delivery Service') {
                if (vendorDetails.hasOwnProperty('workingHours')) {
                    ...
                    // Show store timings
                    showStoreTimings(vendorDetails);
                }
            }
        ...
        if (vendorProduct != undefined) {
            ...
            if (vendorProduct.item_attribute != null && vendorProduct.item_attribute != "" && vendorProduct.item_attribute.attributes.length > 0 && vendorProduct.item_attribute.variants.length > 0) {
                ...
                if (vendorProduct.hasOwnProperty('disPrice') && vendorProduct.disPrice != '' && vendorProduct.disPrice != '0' && vendorProduct.item_attribute == null) {
                    ...
                    if (vendorProduct.item_attribute != null && vendorProduct.item_attribute != "" && vendorProduct.item_attribute.attributes.length > 0 && vendorProduct.item_attribute.variants.length > 0) {
                        var variants_prices = [];
                        var variants_dis_prices = [];
                        var variants = vendorProduct.item_attribute.variants;
                        for(variant of variants){
                            variants_prices.push(variant.variant_price);
                            if (variant.variant_dis_price && variant.variant_dis_price !== '') {
                                variants_dis_prices.push(variant.variant_dis_price);
                            }
                        }
                        var min_price = Math.min.apply(Math,variants_prices);
                        var max_price = Math.max.apply(Math,variants_prices);
                        if(variants_dis_prices.length > 0) {
                            var min_dis_price = Math.min.apply(Math,variants_dis_prices);
                            var max_dis_price = Math.max.apply(Math,variants_dis_prices);
                        if(min_price != max_price){
                            var or_price = getProductFormattedPrice(parseFloat(final_price.min)) + " - "+getProductFormattedPrice(parseFloat(final_price.max));
                                var dis_price = getProductFormattedPrice(parseFloat(final_price.min_dis)) + " - "+getProductFormattedPrice(parseFloat(final_price.max_dis));
                        }else{
                            var or_price = getProductFormattedPrice(parseFloat(final_price.max));    
                                var dis_price = getProductFormattedPrice(parseFloat(final_price.max_dis));
                            }
                            var or_price = dis_price + '  <s>' + or_price + '</s>';
                        } else {
                            if(min_price != max_price){
                                var or_price = getProductFormattedPrice(parseFloat(final_price.min)) + " - "+getProductFormattedPrice(parseFloat(final_price.max));
                            }else{
            ...
            html = html + '<h3><?php echo e(trans("lang.store")); ?> | <span class="vendor_name"></span></h3>';
            html = html + '</div>';
            ...
            if (vendorProduct.item_attribute != null && vendorProduct.item_attribute != "" && vendorProduct.item_attribute.attributes.length > 0 && vendorProduct.item_attribute.variants.length > 0) {
                ...
                html = html + '<div class="attribute_list" id="attribute-list-' + vendorProduct.id + '" data-pid="' + vendorProduct.id + '" data-product="' + btoa(unescape(encodeURIComponent(JSON.stringify(vendorProduct)))) + '"></div>';
                html = html + '</div>';
                html = html + '</div>';
            ...
            if (vendorProduct.hasOwnProperty('addOnsPrice') && vendorProduct.addOnsPrice.length > 0) {
                ...
                if (vendorProduct.hasOwnProperty('sizePrice')) {
                    var label_addon = "<?php echo e(trans('lang.addons')); ?>";
                ...
                vendorProduct.addOnsPrice.forEach(async (product_price) => {
                    ...
                    html += '<label class="custom-control-label" for="' + vendorProduct.id + '_extra_' + total + '">' + vendorProduct.addOnsTitle[total] + ' <span class="">+' + getProductFormattedPrice(PRICE) + '</span></label>';
                    html += '</div>';
                    html += '</div>';
                    total++;
                });
            ...
            html = html + '</div>';
            html = html + '</div>';
            html = html + '<div class="not-available mt-5" style="display:none">';
            html = html + '<p class="text-center font-weight-bold h5"><?php echo e(trans("lang.currently_not_available")); ?></p>';
            html = html + '<div class="store-timings-info" style="display:none;">';
            html = html + '<p class="text-center text-muted mb-2">Store Timings:</p>';
            html = html + '<div class="store-timings-list text-center"></div>';
            html = html + '<p class="text-center text-info mt-2">';
            html = html + '<small>Please schedule your order during store hours</small>';
            html = html + '</p>';
            html = html + '</div>';
            html = html + '</div>';
            return html;
    ...
    async function buildHTMLRelatedProducts(snapshots) {
        ...
        html = html + '<h3><?php echo e(trans("lang.related_products")); ?></h3>';
        html = html + '<div class="row">';
        alldata.forEach((listval) => {
            var val = listval;
            var vendor_id_single = val.id;
    ...
    async function getVariantsHtml(vendorProduct, attributes, variants) {
        ...
        if (variation_sku) {
            ...
            var variant_id = variant_image = variant_price = variant_price = variant_quantity = variant_dis_price = '';
            if (variant_info.length > 0) {
                var variant_id = variant_info[0].variant_id;
                var variant_image = variant_info[0].variant_image;
                var variant_price_data = final_price.variants[variant_id];
                var variant_price = variant_price_data.price || variant_price_data;
                var variant_dis_price = variant_price_data.dis_price || null;
                ...
                if (variant_dis_price && variant_dis_price !== null && variant_dis_price !== '') {
                    var or_price = getProductFormattedPrice(parseFloat(variant_price));
                    var dis_price = getProductFormattedPrice(parseFloat(variant_dis_price));
                    var pro_price = dis_price + '  <s>' + or_price + '</s>';
                } else {
                if (currencyAtRight) {
                ...
                $('#variation_info_' + vendorProduct.id).find('#variant_price').attr('data-vdisprice', variant_dis_price || '');
    ...
    function getVariantPrice(vendorProduct) {
        ...
        // Function to show store timings when store is closed
        function showStoreTimings(vendorDetails) {
            if (vendorDetails && vendorDetails.hasOwnProperty('workingHours')) {
                ...
                for (var i = 0; i < vendorDetails.workingHours.length; i++) {
                    ...
                    if (dayTiming.timeslot && dayTiming.timeslot.length > 0) {
                        ...
                        timingsHtml += '<div class="mb-1"><strong>' + dayName + ':</strong> ' + timeSlots.join(", ") + '</div>';
                    }
                }
                
                if (timingsHtml) {
                    $('.store-timings-list').html(timingsHtml);
                    $('.store-timings-info').show();
                }
            }
        }
...
<?php echo $__env->make('layouts.nav', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH E:\ondemand_grihasth-master\ondemand_grihasth-master\resources\views/products/detail.blade.php ENDPATH**/ ?>
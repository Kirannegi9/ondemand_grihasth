        ...
        <div class="search py-5">
            ...
            <ul class="nav nav-tabs border-0" id="myTab" role="tablist">
                <li class="nav-item" role="presentation">
                    <a class="nav-link active border-0 bg-light text-dark rounded" id="home-tab" data-toggle="tab" href="#home" role="tab" aria-controls="home" aria-selected="true"><i class="feather-home mr-2"></i><span class="vendor_counts"><?php echo e(trans('lang.all_store')); ?></span></a>
                </li>
            </ul>
            <ul class="nav nav-tabs border-0" id="myTab" role="tablist">
                <li class="nav-item" role="presentation">
...
<script type="text/javascript">
    ...
    async function getNearestStores() {
        ...
        nearestRestauantRefnew.get().then(async function(nearestRestauantSnapshot) {
            ...
            // Check if we have actual stores, not just empty results
            if (nearestRestauantSnapshot.docs.length > 0) {
                ...
                jQuery("#overlay").hide();
            } else if (VendorNearBy) {
                // If no stores found within radius, fallback to all stores in section
           
                var fallbackRef = geoFirestore.collection('vendors').where('section_id', '==', section_id);
                fallbackRef.get().then(async function(fallbackSnapshot) {
                    var fallbackHtml = buildHTMLNearestStore(fallbackSnapshot);
                    if (fallbackSnapshot.docs.length > 0) {
                        start = fallbackSnapshot.docs[fallbackSnapshot.docs.length - 1];
                        endarray.push(fallbackSnapshot.docs[0]);
                    }
                    most_popular.innerHTML = fallbackHtml;
                    jQuery("#overlay").hide();
                });
            } else {
    ...
    function buildHTMLNearestStore(nearestRestauantSnapshot) {
        ...
        if ('<?php echo @$_GET['popular'] && @$_GET['popular'] == 'yes'; ?>') {
            ...
            if (datas.section_id == "6285ddbfd9598") {
                $('.vendor_counts').text('<?php echo e(trans('lang.popular_restaurant_store')); ?>');
            } else if (datas.section_id == "6285dd7b50f32") {
                $('.vendor_counts').text('<?php echo e(trans('lang.popular_flower_store')); ?>');
            } else if (datas.section_id == "6319dc53314ee") {
                $('.vendor_counts').text('<?php echo e(trans('lang.popular_food_store')); ?>');
            } else {
                $('.vendor_counts').text('<?php echo e(trans('lang.all_store')); ?>');
            }
        }
        var count = 0;
        ...
        alldata.forEach((listval) => {
            ...
            var view_vendor_details = "<?php echo e(route('dyiningvendor', ':id')); ?>";
...
<?php /**PATH E:\ondemand_grihasth-master\ondemand_grihasth-master\resources\views/allvendors/index.blade.php ENDPATH**/ ?>